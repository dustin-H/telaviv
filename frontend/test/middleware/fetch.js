...
describe('The fetch middleware', () => {
  ...
  it('should fetch nothing if fetching data is an empty array', (done) => {
    ...
    mid(req, res, (next) => {
      expect(req.bauhaus.fetch.data).to.be.an('array')
      expect(req.bauhaus.fetch.data.length).to.equal(0)
      done()
    })
  ...
  it('should fetch nothing if fetching data includes no sources', (done) => {
    ...
    mid(req, res, (next) => {
      expect(req.bauhaus.fetch.data).to.be.an('array')
      expect(req.bauhaus.fetch.data.length).to.equal(1)
      expect(req.bauhaus.fetch.data[0].component).to.equal('Footer')
      expect(req.bauhaus.fetch.data[0].data).to.be.an('object')
      expect(JSON.stringify(req.bauhaus.fetch.data[0].data)).to.equal('{}')
      done()
    })
  })
  it('should fetch a simple api source and forward the correct headers and id', (done) => {
    ...
    mid(req, res, (next) => {
      ...
      expect(req.bauhaus.fetch.data).to.be.an('array')
      expect(req.bauhaus.fetch.data.length).to.equal(1)
      expect(req.bauhaus.fetch.data[0].component).to.equal('Header')
      expect(req.bauhaus.fetch.data[0].data).to.be.an('object')
      let d = req.bauhaus.fetch.data[0].data
