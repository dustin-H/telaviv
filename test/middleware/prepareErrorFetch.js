...
describe('The prepareErrorFetch middleware', () => {
  ...
  it('should set clientRendering to false', () => {
    ...
    expect(req.telaviv.clientRendering).to.be.false
    expect(next).to.have.been.calledOnce
    expect(next.args[0].length).to.equal(0)
  })
  ...
  it('should match the correct error route if correct type is available', () => {
    ...
    expect(req.telaviv.fetch).to.be.an('object')
    expect(req.telaviv.fetch.params.code).to.equal('404')
    expect(req.telaviv.fetch.components[0].component).to.equal('SomeComponent')
    expect(next).to.have.been.calledOnce
    expect(next.args[0].length).to.equal(0)
  })
